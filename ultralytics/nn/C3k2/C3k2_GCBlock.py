...
class Bottleneck_GCBlock(Bottleneck):
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        super().__init__(c1, c2, shortcut, g, k, e)
        c_ = int(c2 * e)  # hidden channels
        # 使用GCBlock替换原来的Conv层
        self.cv1 = GCBlock(c1, c_, 3, 1, 1)
        self.cv2 = GCBlock(c_, c2, 3, 1, 1, g=g)
    
    def forward(self, x):
        """Forward pass for Bottleneck_GCBlock using GCBlock layers instead of standard Conv layers."""
        return x + self.cv2(self.cv1(x)) if self.add else self.cv2(self.cv1(x))


class C3k_GCBlock(C3k):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=3):
        super().__init__(c1, c2, n, shortcut, g, e, k)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(Bottleneck_GCBlock(c_, c_, shortcut, g, k=(k, k), e=1.0) for _ in range(n)))


class C3k2_GCBlock(C3k2):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        if c3k:
            self.m = nn.ModuleList(C3k_GCBlock(self.c, self.c, 2, shortcut, g) for _ in range(n))
        else:
            self.m = nn.ModuleList(Bottleneck_GCBlock(self.c, self.c, shortcut, g) for _ in range(n))