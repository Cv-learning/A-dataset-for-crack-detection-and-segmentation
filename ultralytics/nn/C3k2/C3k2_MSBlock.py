...
class C3k_MSBlock(C3):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, k=3):
        ...
        c_ = int(c2 * e)  # hidden channels
        # MSBlock需要  kernel_sizes，这里的[1, 3, 3]就是kernel_sizes的值，可以按需求修改
        self.m = nn.Sequential(*(MSBlock(c_, c_, [1, 3, 3]) for _ in range(n)))

class C3k2_MSBlock(C2f):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        """Initializes the C3k2 module, a faster CSP Bottleneck with 2 convolutions and optional C3k blocks."""
        super().__init__(c1, c2, n, shortcut, g, e)
        # MSBlock需要  kernel_sizes，这里的[1, 3, 3]就是kernel_sizes的值，可以按需求修改
        self.m = nn.ModuleList(
            C3k_MSBlock(self.c, self.c, 2, shortcut, g) if c3k else MSBlock(self.c, self.c,[1, 3, 3]) for _ in range(n)
        )
