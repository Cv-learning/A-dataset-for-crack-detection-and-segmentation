...
def train_model(log_filename=None):
    ...
    try:
        ...
        model = YOLO('yolo11.yaml')  # yolo11n/l/m/s/x可选，n是轻量版，速度最快
        results = model.train(
            data='01_crack.yaml',    # 数据集配置文件路径（确认路径正确）
            epochs=300,              # 训练轮次（搭配早停，无需担心过拟合）
            imgsz=640,               # 输入图片尺寸（适配4090D，无需调整）
            batch=2,               # 批量大小（4090D+320尺寸，64完全适配）
            device=0,                # GPU设备（单卡无需调整）
            name="002_yolo11_origin",     # 子目录名（logs/yolo11_train/），建议加时间戳避免覆盖
            single_cls=True,
            mosaic=0.8,
            amp=False,                 # 关闭混合精度
            workers=8,                 # 数据加载线程
...
if __name__ == '__main__':
    ...
    train_model(log_filename="002_yolo11_origin.txt")