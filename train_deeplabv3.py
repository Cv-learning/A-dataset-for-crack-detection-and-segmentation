...
from deeplabv3_model import DeepLabV3Model
from utils import save_sample_images, save_validation_results

# 设置日志 - 使用固定文件名
log_filename = "deeplabv3_training.log"  # DeepLabV3专用日志文件名
logging.basicConfig(
    level=logging.INFO,
...
class DeepLabV3Trainer:
    def __init__(self):
        ...
        # 初始化模型 - 不使用预训练权重
        self.model = DeepLabV3Model(num_classes=1, pretrained=False).to(self.device)
        ...
        self.writer = SummaryWriter(
            f'runs/deeplabv3_train_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
        )
        ...
        self.csv_path = os.path.join(
            Config.TRAIN_IMAGES_DIR, "deeplabv3_training_metrics.csv"
        )
    ...
    def validate(self, epoch):
        ...
        if validation_images:
            ...
            val_result_path = os.path.join(
                Config.TRAIN_IMAGES_DIR, f"deeplabv3_validation_epoch_{epoch}.png"
            )
    ...
    def save_checkpoint(self, epoch, loss, is_best=False):
        ...
        # 保存当前模型
        current_path = os.path.join(Config.PARAMS_DIR, "deeplabv3_current.pth")
        torch.save(checkpoint, current_path)

        # 保存最佳模型
        if is_best:
            best_path = os.path.join(Config.PARAMS_DIR, "deeplabv3_best.pth")
            torch.save(checkpoint, best_path)
            logger.info(f"Best DeepLabV3 model saved with loss: {loss:.6f}")
    ...
    def save_metrics_to_csv(self):
        ...
        logger.info(f"DeepLabV3 training metrics saved to {self.csv_path}")
    ...
    def plot_training_curves(self):
        ...
        fig.suptitle("DeepLabV3 Training Curves", fontsize=16)
        ...
        plot_path = os.path.join(
            Config.TRAIN_IMAGES_DIR, "deeplabv3_training_curves.png"
        )
        plt.tight_layout()
        plt.savefig(plot_path, dpi=300, bbox_inches="tight")
        logger.info(f"DeepLabV3 training curves saved to {plot_path}")

    def train(self):
        """主训练循环"""
        logger.info("=" * 80)
        logger.info("Starting DeepLabV3 Training (without pretrained weights)")
        ...
        # 尝试加载检查点
        checkpoint_path = os.path.join(Config.PARAMS_DIR, "deeplabv3_current.pth")
        ...
        # 训练完成
        logger.info("=" * 80)
        logger.info("DeepLabV3 training completed!")
        ...
        with torch.no_grad():
            for i, (images, labels) in enumerate(self.val_loader):
                ...
                sample_path = os.path.join(
                    Config.TRAIN_IMAGES_DIR, f"deeplabv3_sample_epoch_{epoch}.png"
                )
                save_sample_images(
    ...
    def save_training_history(self):
        ...
        history_path = os.path.join(
            Config.TRAIN_IMAGES_DIR, "deeplabv3_training_history.json"
        )
        with open(history_path, "w") as f:
            json.dump(history, f, indent=2)

        logger.info(f"DeepLabV3 training history saved to {history_path}")
