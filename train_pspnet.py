...
from pspnet_model import PSPNetSimple
from utils import save_sample_images, save_validation_results

# 设置日志 - 使用固定文件名
log_filename = "pspnet_training.log"  # PSPNet专用日志文件名
logging.basicConfig(
    level=logging.INFO,
...
class PSPNetTrainer:
    def __init__(self):
        ...
        # 初始化模型 - 使用PSPNetSimple
        self.model = PSPNetSimple(num_classes=1).to(self.device)
        ...
        self.writer = SummaryWriter(
            f'runs/pspnet_train_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
        )
        ...
        self.csv_path = os.path.join(
            Config.TRAIN_IMAGES_DIR, "pspnet_training_metrics.csv"
        )
    ...
    def validate(self, epoch):
        ...
        if validation_images:
            ...
            val_result_path = os.path.join(
                Config.TRAIN_IMAGES_DIR, f"pspnet_validation_epoch_{epoch}.png"
            )
    ...
    def save_checkpoint(self, epoch, loss, is_best=False):
        ...
        # 保存当前模型
        current_path = os.path.join(Config.PARAMS_DIR, "pspnet_current.pth")
        torch.save(checkpoint, current_path)

        # 保存最佳模型
        if is_best:
            best_path = os.path.join(Config.PARAMS_DIR, "pspnet_best.pth")
            torch.save(checkpoint, best_path)
            logger.info(f"Best PSPNet model saved with loss: {loss:.6f}")
    ...
    def save_metrics_to_csv(self):
        ...
        logger.info(f"PSPNet training metrics saved to {self.csv_path}")
    ...
    def plot_training_curves(self):
        ...
        fig.suptitle("PSPNet Training Curves", fontsize=16)
        ...
        plot_path = os.path.join(
            Config.TRAIN_IMAGES_DIR, "pspnet_training_curves.png"
        )
        plt.tight_layout()
        plt.savefig(plot_path, dpi=300, bbox_inches="tight")
        logger.info(f"PSPNet training curves saved to {plot_path}")

    def train(self):
        """主训练循环"""
        logger.info("=" * 80)
        logger.info("Starting PSPNet Training")
        ...
        # 尝试加载检查点
        checkpoint_path = os.path.join(Config.PARAMS_DIR, "pspnet_current.pth")
        ...
        # 训练完成
        logger.info("=" * 80)
        logger.info("PSPNet training completed!")
        ...
        with torch.no_grad():
            for i, (images, labels) in enumerate(self.val_loader):
                ...
                sample_path = os.path.join(
                    Config.TRAIN_IMAGES_DIR, f"pspnet_sample_epoch_{epoch}.png"
                )
                save_sample_images(
    ...
    def save_training_history(self):
        ...
        history_path = os.path.join(
            Config.TRAIN_IMAGES_DIR, "pspnet_training_history.json"
        )
        with open(history_path, "w") as f:
            json.dump(history, f, indent=2)

        logger.info(f"PSPNet training history saved to {history_path}")


def main():
    trainer = PSPNetTrainer()
    trainer.train()
