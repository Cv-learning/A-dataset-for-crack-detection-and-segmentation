...
# 设置日志 - 使用固定文件名
log_filename = "unet_training.log"  # 固定日志文件名
logging.basicConfig(
    level=logging.INFO,
...
class Trainer:
    def __init__(self):
        ...
        self.writer = SummaryWriter(
            f'runs/unet_train_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
        )

        # 指标记录
        self.train_metrics = {"epoch": [], "loss": [], "iou": [], "dice": [], "lr": []}
        self.val_metrics = {"epoch": [], "loss": [], "iou": [], "dice": []}

        # CSV文件路径
        self.csv_path = os.path.join(Config.TRAIN_IMAGES_DIR, "training_metrics.csv")
    ...
    def validate(self, epoch):
        ...
        if validation_images:
            ...
            val_result_path = os.path.join(
                Config.TRAIN_IMAGES_DIR, f"validation_epoch_{epoch}.png"
            )
    ...
    def save_checkpoint(self, epoch, loss, is_best=False):
        ...
        # 保存当前模型
        current_path = os.path.join(Config.PARAMS_DIR, "unet_current.pth")
        torch.save(checkpoint, current_path)

        # 保存最佳模型
        if is_best:
            best_path = os.path.join(Config.PARAMS_DIR, "unet_best.pth")
            torch.save(checkpoint, best_path)
            logger.info(f"Best model saved with loss: {loss:.6f}")
    ...
    def plot_training_curves(self):
        ...
        # 保存图像
        plot_path = os.path.join(Config.TRAIN_IMAGES_DIR, "unet_training_curves.png")
        plt.tight_layout()
    ...
    def train(self):
        ...
        # 尝试加载检查点
        checkpoint_path = os.path.join(Config.PARAMS_DIR, "unet_current.pth")
        ...
        with torch.no_grad():
            for i, (images, labels) in enumerate(self.val_loader):
                ...
                sample_path = os.path.join(
                    Config.TRAIN_IMAGES_DIR, f"sample_epoch_{epoch}.png"
                )
                save_sample_images(
